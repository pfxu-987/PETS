...
#        manual_ids=[0 for _ in range(16)]+[i for i in range(32)]
    ...
    def run(self, no_log = False, bs = 32 ):
        ...
        #     if i==0:
        #         turbo_transformers.set_task_to_stream_mapping(i, 0)  # 将任务0映射到流i、
        #     else:
        #         turbo_transformers.set_task_to_stream_mapping(i, i%self.cfg.num_streams)  # 将任务0映射到流i、
    ...
    def compare_multi_stream(self):
        ...
        self.cfg.num_tasks = 4
        self.init()
        self.cfg.total_queries = 1024
        for task_num in [4]:
            self.cfg.num_tasks = task_num
            for i in range(1):
                for seq_len in [4,16,32,64,96,160,256]:
                    self.cfg.mean_v = seq_len
                    self.cfg.std_v = 0
                    self.prepare_query_pool()
                    for b_s in [16,32,64,96,128,192,256]:
                        for stream_num in [32,16,8,4,2,1]:
                            self.cfg.num_streams = stream_num
                            self.write_log("total_queries:{},task_num:{},stream_num:{},batch_size:{},seq_len:{} ".format(self.cfg.total_queries, self.cfg.num_tasks, stream_num,b_s,seq_len))
                            self.run(bs = b_s)
